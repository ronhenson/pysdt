...
def get_movies():
    ...
    tree = get_tree()
    return list(movie.attrib['title'] for movie in list(tree.iter(tag='movie')))
...
def get_movie_longest_runtime():
    ...
    for movie in list(tree.iter(tag='movie')):
        movie_runtime[movie.attrib['title']] = movie.attrib['runtime'].split()[0]

    return max(movie_runtime, key=movie_runtime.get)
